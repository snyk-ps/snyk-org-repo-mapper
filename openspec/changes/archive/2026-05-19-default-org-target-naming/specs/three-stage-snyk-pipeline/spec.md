...
The snyk-import command SHALL read the Stage 1 intermediate document, SHALL build import `targets` compatible with the Snyk import tool, and SHALL query the **Snyk REST API** to set **`orgId`** and **`integrationId`** on each target. For each target, when the Bitbucket `projectKey` appears in the Stage 1–derived `projectKey → apm_code` map with a non-empty `apm_code`, the command SHALL resolve the Snyk organization whose **name** equals that `apm_code` and SHALL select the **Bitbucket Server** integration for that org, and SHALL set `target.name` to the discovery row’s **repository display name** when present, otherwise the repository slug, without a project-key prefix. When the `projectKey` has **no** entry in that map (including when every repository row under the project has null or empty `apm_code`), the command SHALL fail with a clear validation error **unless** the user supplies an optional **default Snyk organization identifier**; when supplied, the command SHALL verify that identifier refers to an organization in the configured Snyk group, SHALL assign that value as **`orgId`**, SHALL assign the **Bitbucket Server** **`integrationId`** for that organization, and SHALL set `target.name` to `{projectKey}/{repository_name}` where `repository_name` is the discovery row’s display name or the repository slug when the display name is absent.
...
#### Scenario: Composite target name for default organization

- **GIVEN** two import targets in different Bitbucket `projectKey` values that share the same repository display name
- **AND** both `projectKey` values are absent from the `projectKey → apm_code` map
- **AND** the user passes a valid default Snyk organization identifier
- **WHEN** Stage 3 builds the import document
- **THEN** the first target’s `name` SHALL be `{firstProjectKey}/{displayName}`
- **AND** the second target’s `name` SHALL be `{secondProjectKey}/{displayName}`
- **AND** the two `name` values SHALL differ

#### Scenario: APM-mapped targets keep unprefixed names

- **GIVEN** an import target whose `projectKey` maps to a non-empty `apm_code`
- **WHEN** Stage 3 builds the import document
- **THEN** `target.name` SHALL equal the repository display name or slug only
- **AND** SHALL NOT include a `{projectKey}/` prefix

#### Scenario: Default organization target uses slug when display name absent

- **GIVEN** an import target whose `projectKey` is absent from the `projectKey → apm_code` map
- **AND** the discovery row has no repository display name
- **AND** the user passes a valid default Snyk organization identifier
- **WHEN** Stage 3 builds the import document
- **THEN** `target.name` SHALL be `{projectKey}/{repoSlug}`
