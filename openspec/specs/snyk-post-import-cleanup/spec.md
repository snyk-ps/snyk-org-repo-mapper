...
Stage 4 normalizes every organization in the configured Snyk group after import: deletes Dockerfile projects, disables recurring tests on all projects, preserves pre-PATCH project ownership after recurring-test PATCH, re-applies the Stage 2.3 Bitbucket Server integration settings profile, and sets org Python language settings to 3.12.
