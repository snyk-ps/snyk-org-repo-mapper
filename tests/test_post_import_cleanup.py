...
def test_run_post_import_cleanup_applies_changes() -> None:
    ...
    client.update_project_settings.assert_called_once_with(
        "org-1",
        "p1",
...
def test_run_post_import_cleanup_skips_patch_404() -> None:
    ...
    client.iter_group_orgs.return_value = [{"id": "org-1", "name": "APM1"}]
    client.iter_org_projects.side_effect = [[], [{"id": "p1", "name": "App", "type": "npm"}]]
    client.update_project_settings.side_effect = RuntimeError(
        "Snyk API HTTP 404 for https://api.snyk.io/rest/orgs/org-1/projects/p1: not found"
    )
    client.iter_org_integrations.return_value = [
        {"id": "int-1", "type": "bitbucket-server"}
    ]
...
def test_cli_requires_user_id(monkeypatch) -> None:
    monkeypatch.setenv("SNYK_TOKEN", "t")
    monkeypatch.setenv("SNYK_GROUP_ID", "g")
    monkeypatch.setenv("SNYK_INTEGRATIONS_API", "v1")
    monkeypatch.delenv("SNYK_USER_ID", raising=False)

    rc = cleanup_main(["--output", "out.json"])
    assert rc == 2


def test_cli_allows_dry_run_without_user_id(monkeypatch) -> None:
    monkeypatch.setenv("SNYK_TOKEN", "t")
    monkeypatch.setenv("SNYK_GROUP_ID", "g")
    monkeypatch.setenv("SNYK_INTEGRATIONS_API", "v1")
    monkeypatch.delenv("SNYK_USER_ID", raising=False)
